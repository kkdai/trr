...
func (rc *raftNode) openWAL() *wal.WAL {
	if wal.Exist(rc.waldir) == false {
		if err := os.Mkdir(rc.waldir, 0750); err != nil {
			log.Fatalf("trr: cannot create dir for wal (%v)", err)
		}

		w, err := wal.Create(rc.waldir, nil)
		if err != nil {
			log.Fatalf("trr: create wal error (%v)", err)
		}
		w.Close()
	}
	...
	if err != nil {
		log.Fatalf("trr: error loading wal (%v)", err)
	}
...
func (rc *raftNode) replayWAL() *wal.WAL {
	...
	if err != nil {
		log.Fatalf("trr: failed to read WAL (%v)", err)
	}
...
func (rc *raftNode) serveChannels() {
	...
	for {
		...
		case rd := <-rc.node.Ready():
			...
			//log.Println("raft send:=>", rd.Messages)
...
func (rc *raftNode) serveRaft() {
	...
	//log.Println("URL-->", url, err)
	if err != nil {
		log.Fatalf("trr: Failed parsing URL (%v)", err)
	}

	ln, err := newStoppableListener(url.Host, rc.httpstopc)
	if err != nil {
		log.Fatalf("trr: Failed to listen rafthttp (%v)", err)
	}
	...
	case <-rc.httpstopc:
	default:
		log.Fatalf("trr: Failed to serve rafthttp (%v)", err)
	}
	close(rc.httpdonec)
}
